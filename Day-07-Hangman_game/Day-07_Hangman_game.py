...
for position in range(len(random_select)):
    under_score += "_"
print (under_score)

correct_letters=[]

while "_" in under_score and lives >0:  
    ...
    if user_guess not in random_select:
        lives -= 1
        print(f"You guessed {user_guess}, that's not in the word. You lose a life.")
        if lives == 0:
            print (f"**********It was {random_select} You lose************")
    under_score = display
    print(display)
    print(stages[lives])
if lives > 0: 
